...
void SVC_Handler_Main(unsigned int *svc_args){
	...
	//From Stack to CPU (psp points to end of stack context)
	if(svc_number == 1){
		__asm("MRS R0, PSP");
	...
	//From CPU to Stack (psp points to start of stack context)
	else if(svc_number == 2){

		_ICSR |= 1<<28; //control register bit for a PendSV interrupt
        __asm("ISB"); //Automatically puts first 8 regs into stack
	}
